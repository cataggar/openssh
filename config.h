...
/*
 * NOTE: on the zig16-no-automake branch, this file is a *frozen snapshot*,
 * committed as-is rather than regenerated by autoconf on every build.
 * build.zig no longer runs autoreconf/configure/make -- it compiles every
 * .c file directly, and expects this exact config.h (captured via a real
 * `./configure --with-pam` on x86_64 Linux/glibc, with OpenSSL, zlib and
 * PAM development headers installed).
 *
 * If you need to target a different OS/libc/toolchain, or enable/disable
 * optional features (Kerberos, SELinux, etc.), regenerate this file with
 * autoconf (see the zig16-automake branch, or upstream OpenSSH's normal
 * ./configure flow) and replace it here.
 */
